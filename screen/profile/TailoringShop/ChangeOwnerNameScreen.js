...
const ChangeOwnerNameScreen = props => {
  ...
  useEffect(() => {
    props.navigation.setOptions({
      headerRight: () => (
        <TouchableOpacity onPress={updateOwnerNameHandler}>
          <ScrollView>
  ...
  const updateOwnerNameHandler = async () => {
    Keyboard.dismiss();
    ...
    <ScrollView
      ...
      <Card style={styles.itemsContainer}>
        <CustomInputWithLabel
          ...
          required
          isError={inputError}
          labelText="Enter Store Owner*"
          placeHolder="Enter Store Owner"
          errorText="Invalid Store Owner"
        ...
        />
        <MainButton
          onPress={updateOwnerNameHandler}
          label={storeInfo.storeOwner === '' ? 'SUBMIT' : 'UPDATE'}
        />
      </Card>
    </ScrollView>
  );
...
export default ChangeOwnerNameScreen;
